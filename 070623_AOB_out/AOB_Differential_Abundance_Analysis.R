...
set.seed(13)
aob.physeqr1k <- rarefy_even_depth(aob.physeq, sample.size = 1282,rngseed =13, replace = F, trimOTUs = TRUE, verbose = TRUE)
sort(sample_sums(aob.physeqr1k), decreasing = F) # 123 ASVs were removed because they are no longer present in any sample after random subsampling
                                                 # 1 sample was removed (S11)

################################
# Filter low-abundant taxa
###############################
aob.physeq.subset <- aob.physeqr1k
aob1k.df <- as.data.frame(otu_table(aob.physeqr1k))

### keeping ASVs with at least 0.01 % relative abundance across all samples
keep.taxa.id=which((rowSums(aob1k.df)/sum(aob1k.df))>0.0001)
aob.filt.df=aob1k.df[keep.taxa.id,,drop=FALSE]
...
aob.physeq.subset
# # it means only 474 taxa remain in the dataset after filtering. 

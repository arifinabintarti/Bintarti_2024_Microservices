...
### Mantel correlation analysis with Spearman's correlation method
...
aob.t1.cont <- trans_env$new(dataset = aob.cont_microdata, env_cols = c(17:18,20:21,23:24,27,31:33,51:60))
set.seed(13)
...
aoa.t1.cont <- trans_env$new(dataset = aoa.cont_microdata, env_cols = c(17:18,20:21,23:24,27,31:33,51:60),complete_na = TRUE) 
set.seed(13)
...
set.seed(13)
# set the complete_na=TRUE becuse there is NA, if not it would not working, removal is not an option because then i need to remove everything from the otu table
com.t1.cont <- trans_env$new(dataset = com.cont_microdata, env_cols = c(17:18,20:21,23:24,27,31:33,51:60), complete_na = TRUE)
set.seed(13)
...
ggsave("simplemantel.control_withN2O_Spearman.tiff",
...
extra.params <- extra_params(spec.label = text_params(size = 7,vjust=1.5, hjust=0.6),
                             spec.point = point_params(size=2,fill = "grey28"))
                             #link.params = link_params(spec.point.hjust = 2))
mantelplot.control.noAOBalph <- quickcor(all.env.cont.ed3, type = "upper", cor.test = TRUE,method="spearman",exact=FALSE,use="pairwise.complete.obs") + #cor.test = TRUE #use="complete.obs"
  ...
  #geom_raster()+
  geom_circle2(position = "identity")+
  #labs(subtitle="A. Control")+
  geom_mark(sig.thres = 0.05, color = "grey90", size=5, r=NA, fontface="bold", hjust=0.5, vjust=0.5) +
  add_link(plot_table.CONT, mapping = aes(colour = pd, size = rd),extra.params = extra.params, #on.left = F,
           diag.label = F,
           spec.label.hspace = 1.5, #0.75
           spec.label.vspace = -0.8) + #-0.3
...
ggsave("simplemantel.control.noAOBalph_withN2O_Spearman.tiff",
       mantelplot.control.noAOBalph, device = "tiff",
       width = 14.6, height = 8.5,
       #width = 13, height = 10, 
       units= "in", dpi = 600)
...
set.seed(13)
aob.t1.dro <- trans_env$new(dataset = aob.dro_microdata, env_cols = c(17:18,20:21,23:24,27,31:33,51:60),complete_na = TRUE) # interpolation for the missing values
set.seed(13)
...
set.seed(13)
# set the complete_na=TRUE becuse there is NA, if not it would not working, removal is not an option because then i need to remove everything from the otu table
aoa.t1.dro <- trans_env$new(dataset = aoa.dro_microdata, env_cols = c(17:18,20:21,23:24,27,31:33,51:60),complete_na = TRUE) 
set.seed(13)
...
setwd('D:/Fina/INRAE_Project/microservices_fig/')
setwd('/Users/arifinabintarti/Documents/France/Figures')
ggsave("simplemantel.drought_withN2O_Spearman.tiff",
...
extra.params <- extra_params(spec.label = text_params(size = 7,vjust=1.5, hjust=0.6),
                             spec.point = point_params(size=2,fill = "grey28"))
                             #link.params = link_params(spec.point.hjust = 2))
mantelplot.drought.noCOMalph <- quickcor(all.env.dro.ed3, type = "upper", cor.test = TRUE,method="spearman",exact=FALSE,use="pairwise.complete.obs") + #cor.test = TRUE #use="complete.obs"
  #geom_square(position = "identity") + 
  #geom_raster()+
  geom_circle2(position = "identity")+
  #labs(subtitle="B. Drought")+
  geom_mark(sig.thres = 0.05, color = "grey90", size=5, r=NA, fontface="bold", hjust=0.5, vjust=0.5) +
  add_link(plot_table.DRO, mapping = aes(colour = pd, size = rd),extra.params = extra.params, #on.left = F,
           diag.label = F,
           spec.label.hspace = 1.5, #0.75
           spec.label.vspace = -0.8) + #-0.3
           #spec.label.hspace = 0.75,
...
ggsave("simplemantel.drought.noCOMalph_withN2O_Spearman.tiff",
       mantelplot.drought.noCOMalph, device = "tiff",
       width = 14.6, height = 8.5, 
       units= "in", dpi = 600)

ALL.mantel <- mantelplot.control.noAOBalph | mantelplot.drought.noCOMalph
ALL.mantel
ggsave("ALL.mantel.tiff",
       ALL.mantel, device = "tiff",
       width = 12, height = 8.1, 
       units= "in", dpi = 600)
...
legend <- cowplot::get_legend(mantelplot.control.noAOBalph + theme(legend.position = "bottom"))
ALL.mantel <- ggarrange(mantelplot.control.noAOBalph, mantelplot.drought.noCOMalph,
                    labels = c("A. Control", "B. Drought"),
                    nrow = 2)
cowplot::plot_grid(mantelplot.control.noAOBalph, mantelplot.drought.noCOMalph, ncol=2, legend)
